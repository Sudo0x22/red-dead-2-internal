...
#include <winnt.h>
#include<Windows.h>
#include "../../../../injector/protection/xorstr.h"
...
NTSTATUS WINAPI MyZwQueryVirtualMemory(HANDLE  ProcessHandle, PVOID BaseAddress, MEMORY_INFORMATION_CLASS MemoryInformationClass, PVOID MemoryInformation, SIZE_T MemoryInformationLength, PSIZE_T ReturnLength)
{
	...
	if (NT_SUCCESS(nt))
	{
		if (ProcessHandle == pWinApiEx->MyRtlGetCurrentProcess() || pWinApiEx->MyRtlGetProcId(ProcessHandle) == pWinApiEx->MyRtlGetCurrProcId())
		{
			if (BaseAddress >= global::g_hInst && BaseAddress <= (global::g_hInst + global::g_dwModuleSize))
			{
				if (MemoryInformationClass == MemoryBasicInformation)
				{
					////printf("MyZwQueryVirtualMemory->MemoryBasicInformation 0x%llX",BaseAddress);
				...
				{
					////printf("MyZwQueryVirtualMemory->MemorySectionName");
					PUNICODE_STRING SectionName = (PUNICODE_STRING)MemoryInformation;
					if (SectionName)
					{
						////printf("SectionName:%s", SectionName->Buffer);
...
NTSTATUS NTAPI MyZwQueryInformationThread(IN HANDLE ThreadHandle, IN THREADINFOCLASS ThreadInformationClass, PVOID ThreadInformation, _In_ ULONG ThreadInformationLength, _Out_opt_ PULONG ReturnLength)
{
	if (ThreadQuerySetWin32StartAddress == ThreadInformationClass)
	{
		if (ThreadInformation >= global::g_hInst && ThreadInformation <= (global::g_hInst + global::g_dwModuleSize))
		{
			...
			//这可不行
			//printf("MyZwQueryInformationThread: 0x%llX", ThreadInformation);
			return STATUS_UNSUCCESSFUL;
		}
	}
...
{
	if (pWinApiEx->MyRtlGetCurrentProcess() != ProcessHandle)
	{
		DWORD dwPID = pWinApiEx->MyRtlGetProcId(ProcessHandle);
		if (dwPID != pWinApiEx->MyRtlGetCurrProcId())
		{
	...
	if (BaseAddress >= global::g_hInst && BaseAddress <= (global::g_hInst + global::g_dwModuleSize))
	{
		//printf("MyZwReadVirtualMemory: 0x%llX", BaseAddress);
		return STATUS_UNSUCCESSFUL;
	}
	return Old_ZwReadVirtualMemory(ProcessHandle, BaseAddress, Buffer, NumberOfBytesToRead, NumberOfBytesReaded);
}
...
NTSTATUS NTAPI MyZwProtectVirtualMemory(IN HANDLE ProcessHandle, IN PVOID* BaseAddress, IN SIZE_T* NumberOfBytesToProtect, IN ULONG NewAccessProtection, OUT PULONG OldAccessProtection)
{
	...
	{
		//printf("MyZwProtectVirtualMemory:%llX", BaseAddress);
		return STATUS_UNSUCCESSFUL;
	}
...
BOOL WINAPI MyEnumProcessModulesEx(HANDLE  hProcess, HMODULE* lphModule, DWORD cb, LPDWORD lpcbNeeded, DWORD dwFilterFlag)
{
	...
	if (bRet)
	{
		if (hProcess == pWinApiEx->MyRtlGetCurrentProcess() || pWinApiEx->MyRtlGetProcId(hProcess) == pWinApiEx->MyRtlGetCurrProcId())
		{
			//printf("MyEnumProcessModulesEx");
		}
	}
	return bRet;
}
...
{
	//	//printf("EAC Memory Scaning: 0x%llX", lpAddress);
	...
	{
		////printf("MyVirtualQuery: 0x%llX", lpAddress);
...
__int64 WINAPI MyRtlUserThreadStart(PTHREAD_START_ROUTINE pfnStartAddr, PVOID pvParam)
{
	...
	{
		//自己的不经过EAC的
		//printf("MyRtlUserThreadStart My Thread! 0x%11X", pfnStartAddr);
		auto hModule = pWinApi->GetModuleName(__("kernel32.dll"));
		static _BaseThreadInitThunk BaseThreadInitThunk = (_BaseThreadInitThunk)pWinApi->GetModuleProcAddr(hModule, __("BaseThreadInitThunk"));
...
BOOL MyThread32Next(HANDLE hSnapshot, LPTHREADENTRY32 lpte)
{
	...
	{
		//printf("MyThread32Next EAC试图查询线程ID! %d", lpte->th32ThreadID);
...
HANDLE MyOpenThread(_In_ DWORD dwDesiredAccess, _In_ BOOL bInheritHandle, _In_ DWORD dwThreadId)
{
	...
	{
		//printf("MyOpenThread EAC试图打开线程! %d", dwThreadId);
		threadID = 0;
	}
...
void StartHook()
{
	HMODULE hNtDll = pWinApi->GetModuleName(__("ntdll.dll"));
	HMODULE hKEDll = pWinApi->GetModuleName(__("kernel32.dll"));
	if (hNtDll != NULL)
	{
		_NtQueryVirtualMemory ZwQueryVirtualMemory = (_NtQueryVirtualMemory)pWinApi->GetModuleProcAddr(hNtDll, __("NtQueryVirtualMemory"));
		_ZwReadVirtualMemory ZwReadVirtualMemory = (_ZwReadVirtualMemory)pWinApi->GetModuleProcAddr(hNtDll, __("ZwReadVirtualMemory"));
		_ZwProtectVirtualMemory ZwProtectVirtualMemory = (_ZwProtectVirtualMemory)pWinApi->GetModuleProcAddr(hNtDll, __("ZwProtectVirtualMemory"));
		_RtlGetNativeSystemInformation RtlGetNativeSystemInformation = (_RtlGetNativeSystemInformation)pWinApi->GetModuleProcAddr(hNtDll, __("RtlGetNativeSystemInformation"));

		_VirtualQuery VirtualQuery = (_VirtualQuery)pWinApi->GetModuleProcAddr(hKEDll, __("VirtualQuery"));
		_IsBadReadPtr IsBadReadPtr = (_IsBadReadPtr)pWinApi->GetModuleProcAddr(hKEDll, __("IsBadReadPtr"));

		_ZwQueryInformationThread ZwQueryInformationThread = (_ZwQueryInformationThread)pWinApi->GetModuleProcAddr(hNtDll, __("ZwQueryInformationThread"));
		_RtlUserThreadStart RtlUserThreadStart = (_RtlUserThreadStart)pWinApi->GetModuleProcAddr(hNtDll, __("RtlUserThreadStart"));

		if (MH_CreateHookApi(L"kernel32.dll", __("Thread32Next"), &MyThread32Next, reinterpret_cast<void**>(&Old_Thread32Next)) != MH_OK)
		{
			//printf("失败Thread32Next");
			return;
		}
		...
		if (MH_CreateHookApi(L"ntdll.dll", __("ZwQueryVirtualMemory"), &MyZwQueryVirtualMemory, reinterpret_cast<void**>(&Old_ZwQueryVirtualMemory)) != MH_OK)
		{
			//printf("失败ZwQueryVirtualMemory");
			return;
		}
		...
		{
			//printf("失败ZwReadVirtualMemory");
			return;
		}
		if (MH_CreateHookApi(L"ntdll.dll", __("ZwProtectVirtualMemory"), &MyZwProtectVirtualMemory, reinterpret_cast<void**>(&Old_ZwProtectVirtualMemory)) != MH_OK)
		{
			//printf("失败ZwProtectVirtualMemory");
			return;
		}
		if (MH_CreateHookApi(L"ntdll.dll", __("ZwQueryInformationThread"), &MyZwQueryInformationThread, reinterpret_cast<void**>(&Old_ZwQueryInformationThread)) != MH_OK)
		{
			//printf("失败MyZwQueryInformationThread");
			return;
		}
		if (MH_CreateHookApi(L"ntdll.dll", __("RtlGetNativeSystemInformation"), &MyRtlGetNativeSystemInformation, reinterpret_cast<void**>(&Old_RtlGetNativeSystemInformation)) != MH_OK)
		{
			//printf("失败RtlGetNativeSystemInformation");
			return;
		}

		if (MH_CreateHookApi(L"kernel32.dll", __("IsBadReadPtr"), &MyIsBadReadPtr, reinterpret_cast<void**>(&Old_IsBadReadPtr)) != MH_OK)
		{
			//printf("失败IsBadReadPtr");
			return;
		}

		if (MH_CreateHookApi(L"kernel32.dll", __("VirtualQuery"), &MyVirtualQuery, reinterpret_cast<void**>(&Old_VirtualQuery)) != MH_OK)
		{
			//printf("失败VirtualQuery");
			return;
		}

		HMODULE hPsapi = LoadLibraryA("psapi.dll");
		_EnumProcessModulesEx EnumProcessModulesEx = (_EnumProcessModulesEx)pWinApi->GetModuleProcAddr(hPsapi, "EnumProcessModulesEx");
		...
		{
			//printf("Thread32Next 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = MH_EnableHook(&OpenThread)) != MH_OK)
		{
			//printf("OpenThread 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = MH_EnableHook(ZwQueryVirtualMemory)) != MH_OK)
		{
			//printf("ZwQueryVirtualMemory 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = MH_EnableHook(ZwReadVirtualMemory)) != MH_OK)
		{
			//printf("ZwReadVirtualMemory 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = MH_EnableHook(ZwProtectVirtualMemory)) != MH_OK)
		{
			//printf("ZwProtectVirtualMemory 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = MH_EnableHook(RtlGetNativeSystemInformation)) != MH_OK)
		{
			//printf("RtlGetNativeSystemInformation 失败:%s", MH_StatusToString(sts));
			return;
		}

		if ((sts = MH_EnableHook(&IsBadReadPtr)) != MH_OK)
		{
			//printf("IsBadReadPtr 失败:%s", MH_StatusToString(sts));
			return;
		}

		if ((sts = MH_EnableHook(VirtualQuery)) != MH_OK)
		{
			//printf("VirtualQuery 失败:%s", MH_StatusToString(sts));
			return;
		}
		if ((sts = MH_EnableHook(ZwQueryInformationThread)) != MH_OK)
		{
			//printf("ZwQueryInformationThread 失败:%s", MH_StatusToString(sts));
			return;
		}

		if ((sts = MH_EnableHook(RtlUserThreadStart)) != MH_OK)
		{
			//printf("RtlUserThreadStart 失败:%s", MH_StatusToString(sts));
			return;
		}

		if ((sts = MH_EnableHook(EnumProcessModulesEx)) != MH_OK)
		{
			//printf("失败:%s", MH_StatusToString(sts));
			return;
		}
	}
}
